...
class ServiceRoutingTests(unittest.TestCase):
    def test_default_service_type_is_api(self) -> None:
        self.assertEqual(resolve_root_urlconf(None), "ufc_fantasy.public_urls")
        self.assertEqual(resolve_root_urlconf(""), "ufc_fantasy.public_urls")
        self.assertEqual(resolve_root_urlconf("   "), "ufc_fantasy.public_urls")

    def test_api_service_type_resolves_public_urls(self) -> None:
        self.assertEqual(resolve_root_urlconf("api"), "ufc_fantasy.public_urls")

    def test_unsupported_service_type_fails_fast(self) -> None:
        with self.assertRaisesRegex(ValueError, "Unsupported SERVICE_TYPE"):
            resolve_root_urlconf("not-a-real-service")

    def test_worker_service_types_resolve_to_dedicated_urlconfs(self) -> None:
        self.assertEqual(
            resolve_root_urlconf("fights_in_event"),
            "ufc_fantasy.fights_in_event_urls",
        )
        self.assertEqual(
            resolve_root_urlconf("fighter_profile"),
            "ufc_fantasy.fighter_profile_urls",
        )
        self.assertEqual(
            resolve_root_urlconf("fight_stats"),
            "ufc_fantasy.fight_stats_urls",
        )
        self.assertEqual(
            resolve_root_urlconf("career_stats"),
            "ufc_fantasy.career_stats_urls",
        )
        self.assertEqual(
            resolve_root_urlconf("score_fight"),
            "ufc_fantasy.score_fight_urls",
        )

    def test_unknown_service_type_still_fails_fast(self) -> None:
        with self.assertRaisesRegex(ValueError, "Unsupported SERVICE_TYPE"):
            resolve_root_urlconf("pipeline")

    def test_public_urlconf_module_is_api_only(self) -> None:
        ...
        self.assertIn("api.urls", source)
        self.assertNotIn("pipeline", source)
        self.assertNotIn("pubsub", source)
